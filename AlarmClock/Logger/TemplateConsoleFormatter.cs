...
public class TemplateConsoleFormatter : ConsoleFormatter
{
    ...
    {
        _template = Regex.Replace(options.Value.Template, $"{{{LogLevelName}(.*)}}", "{0$1}");
        _template = Regex.Replace(_template, $"{{{TimestampName}(.*)}}", "{1$1}");
        _template = Regex.Replace(_template, $"{{{CategoryName}(.*)}}", "{2$1}");
        _template = Regex.Replace(_template, $"{{{ShortCategoryName}(.*)}}", "{3$1}");
        _template = Regex.Replace(_template, $"{{{MessageName}(.*)}}", "{4$1}");
    }
        ...
        int eventId, string? exception, string category, DateTimeOffset stamp)
    {
        ...
        var resultMessage = string.Format(_template, logLevelName, stamp, category, shortCategory, message);
